...
int main() {
    ...
    robot_loc.pos.x = 4;
    robot_loc.pos.y = 4;
    while (robot_loc.pos.x != 2 && robot_loc.pos.y != 1){
        ...
        //printTree(root, 0);
        BetterChoice choice = calculatebetter(root);
        printf("\nMeilleur prix a %d au position x = %d et y = %d",choice.cost,choice.loc.pos.x,choice.loc.pos.y);
